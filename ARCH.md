...
八卦算子用预先赋予的语义（乾=圆、坤=平、坎=陷）去检测图像特征。但"圆""平""陷"在2D图像中是人为解释，不是物理事实。算子层面输出的是解释而不是测量——导致无法突破表象→本质的鸿沟。

白瓷杯、透明玻璃杯、金属杯在表象上完全不同，但都是"容器"。八卦算子测不到"容器"——乾测出圆口、坤测出瓷面平滑、坎测暗度，没有一个算子输出"能装东西"。因为"能装东西"是物理功能，不是像素统计。

### 新方案：物理本质空间

8个算子不再承载卦象语义，改为测量可验证的物理量：
- 阳（存在）：像素确信是实体内部
- 阴（虚空）：像素确信是外部或空腔
- 动（变化）：梯度/突变强度
- 静（恒定）：无变化区域
...
理解不靠算子语义，靠物理推演——粒子在聚内被困→容器、刚边界反弹→固体、柔边界穿透→软体。这是可验证的物理事实，不是人为解释。

### 旧架构保留

八卦→64卦实现在 `src/operators.py` + `src/pipeline.py` 中。切换到新物理算子后，旧算子归档为 `archived/`。
